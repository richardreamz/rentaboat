...
var img_close;
img_del=new Image();
img_del.src="imagescal/btn_del_small.gif";
img_close=new Image();
img_close.src="imagescal/btn_close_small.gif";

//function for getting current date
function getTodaydate(NowfrmName,NowDateBox)
	{
		eval('document.' + NowfrmName + '.' + NowDateBox + '.value = "'+ padout(todayDate.getDate()) + '-' + names[todayDate.getMonth()] + '-' + todayDate.getFullYear() + '"');
	}
...
img_Date_UP.src="imagescal/calendar1.gif";

img_Date_OVER=new Image();
img_Date_OVER.src="imagescal/calendar1.gif";

img_Date_DOWN=new Image();
img_Date_DOWN.src="imagescal/calendar1.gif";
...
function showCalendar(frmName, dteBox,btnImg, hideDrops, MnDt, MnMo, MnYr, MxDt, MxMo, MxYr,runFuncs) {
    ...
    /////////////////////////
    //Added by Mahendra 12-Mar-02. Set the default month and year to the month and year in the text box.
    //First Parse the date.
    if(document.all[dteBox].value.length > 0)
    {
       var strDt
       var dtInMilliSecs
       
       strDt = document.all[dteBox].value.replace("-"," ").replace("/"," ").replace("-"," ").replace("/"," ")
       dtInMilliSecs = Date.parse (strDt)
       var mm
       var yyyy
    
       var dt = new Date(dtInMilliSecs)
       mm = dt.getMonth()
       yyyy = dt.getFullYear()
    ...
    if (IsCalendarVisible) {
        ...
        /////////////////////////
        //Added by Mahendra 12-Mar-02. Set the default month and year to the month and year in the text box.
        domlay('popupcalendar',1,ppcX,ppcY,Calendar(mm,yyyy));
        /////////////////////////
...
function domlay(id,trigger,lax,lay,content) {
    ...
    if (trigger=="1"){
        ...
        else if (document.all) document.all['popupcalendar'].style.visibility = "visible"
...
function Calendar(whatMonth,whatYear) {
    ...
    output += '<td class="cal-HeadCell" align="center" width="100%"><a href="javascript:clearDay();"><img name="calbtn1" src="imagescal/btn_del_small.gif" border="0" width="12" height="10"></a>&nbsp;&nbsp;<a href="javascript:scrollMonth(-1);" class="scrollmonth">&lt;</a>&nbsp;<SELECT class="cal-TextBox" NAME="cboMonth" onChange="changeMonth();">';
    ...
    output += '<\/SELECT>&nbsp;<a href="javascript:scrollMonth(1);" class="scrollmonth">&gt;</a>&nbsp;&nbsp;<a href="javascript:hideCalendar();"><img name="calbtn2" src="imagescal/btn_close_small.gif" border="0" width="12" height="10"></a><\/td><\/tr><tr><td width="100%" align="center">';
...
curDate.setDate(whatDay);
    eval('document.' + calfrmName + '.' + curDateBox + '.value = "'+ names[curDate.getMonth()] + '-' + padout(curDate.getDate())  + '-' + curDate.getFullYear() + '"');

 eval('document.' + calfrmName + '.TxtToDate.value = "'+ names[curDate.getMonth()] + '-' + padout(curDate.getDate())  + '-' + curDate.getFullYear() + '"');
