  ...
  public partial class facilities_activate_aspx_cs : System.Web.UI.Page
  {
    ...
    public string ConvierteFecha(ref string sStartDate) 
    {
        ...
        public void sendEmail(String marinaID)
        {
            ...
            DataTable dt = Util.getDataSet("select vc_contactname, vc_email from TBL_BR_MARINA where in_MarinaID=" + marinaID ).Tables[0];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //  DataTable dtUser = Util.getDataSet("select vc_username, vc_password from TBL_BR_User where vc_username='" + dt.Rows[0]["vc_email"].ToString() + "'" ).Tables[0];
                DataTable dtUser = Util.getDataSet("select vc_username, vc_password from TBL_BR_User where in_MarinaID=" + marinaID ).Tables[0];
                ...
                body = body.Replace("{BoatOwnerName}", dt.Rows[0]["vc_contactname"].ToString());
                ...
                { 
                    ...
                    string pass = dtUser.Rows[0]["vc_password"].ToString();
                    int len = pass.Length;
                    if (len > 3)
                    {
                    ...
                    body = body.Replace("{Passsword}", pass);
                ...
                Util.LogEMail("info@rentaboat.com", dt.Rows[0]["vc_email"].ToString() + ",info@rentaboat.com,enngines@aol.com", "RentABoat.com account is Approved !", body, "Account Activation");



                Util.SendEMail("info@rentaboat.com", dt.Rows[0]["vc_email"].ToString() + ",info@rentaboat.com,enngines@aol.com", "RentABoat.com account is Approved !", body);
