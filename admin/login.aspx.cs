...
namespace BoatRenting {
  ...
  public partial class login_aspx_cs : System.Web.UI.Page
  {
    ...
    public object Login(string Username, string Password) 
    {
        ...
        cmd.Parameters.Append(cmd.CreateParameter("@P_VC_UserName", adVarChar, adParamInput, 20, Username));
        cmd.Parameters.Append(cmd.CreateParameter("@P_VC_Password", adVarChar, adParamInput, 20, Password));
        rs = cmd.Execute();
        if (!(rs.Eof))
        {
            ...
            Session.Add("defaultPage", NVL(Convert.ToString(rs.Fields["vc_defaultHomePage"].Value), "") + dotNET);
        }
        return null;
    }
